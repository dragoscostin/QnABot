...
namespace Microsoft.Bot.Sample.QnABot
{
    ...
    public class BasicQnAMakerDialog : QnAMakerDialog
    {
        ...
        private string SQLStorageSpace;
...
protected override async Task DefaultWaitNextMessageAsync(IDialogContext context, IMessageActivity message, QnAMakerResults result)
        {
            var answer = result.Answers.Count;
            Activity reply = ((Activity)context.Activity).CreateReply();
            if (answer < 1)
            {

                //SQL Logic to store questions
                using (SqlConnection connection = new SqlConnection(SQLStorageSpace))
                {
                    String query = "INSERT INTO dbo.UnansweredQuestions (Question) VALUES (@Question)";

                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@Question", "abc");
